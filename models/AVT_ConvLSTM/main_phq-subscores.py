        ...
        for mode in ['train', 'test']:
            ...
            for data in tqdm(dataloaders[mode]):
                ...
                def model_processing(input):
                    ...
                    # -------------------------------------- features fusion --------------------------------------
                    # # concatenate, output dim: (B, audio_features*2)
                    # features = torch.cat([visual_features, audio_features, text_features], dim=-1)
                    # # sum, output dim: (B, audio_features)
                    # features = visual_features + audio_features + text_features
                    # # mean, output dim: (B, audio_features)
                    # features = (visual_features + audio_features + text_features) / 3
                    # # median, output dim: (B, audio_features)
                    # features = torch.median(torch.stack([visual_features, audio_features, text_features], axis=-1), dim=-1).values
                    # # maximum, output dim: (B, audio_features)
                    # features = torch.stack([visual_features, audio_features, text_features], axis=-1).max(dim=-1).values
                    # multiplication, output dim: (B, audio_features)
                    features = visual_features * audio_features * text_features
                ...
                if mode == 'train':
                    ...
                    if config['OPTIMIZER']['USE_SAM']:
                        models = [visual_net, audio_net, text_net, evaluator]
