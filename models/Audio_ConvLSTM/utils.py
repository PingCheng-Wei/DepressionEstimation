...
from models.convlstm import ConvLSTM_Audio
...
def get_models(model_config, args, model_type=None, ckpt_path=None):
    ...
    if weights_path is not None:
        ...
        if 'audio_net' in model_config['WEIGHTS']['INCLUDED']:
            ...
            # audio_net_dict = audio_net.state_dict()
            # # 1. filter out unnecessary keys
            # checkpoint_audio_net = {k: v for k, v in checkpoint.items() if k in audio_net_dict}
            # # 2. overwrite entries in the existing state dict
            # audio_net_dict.update(checkpoint_audio_net)
            # # 3. load the new state dict
            # audio_net.load_state_dict(audio_net_dict)
