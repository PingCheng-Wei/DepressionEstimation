...
def main(dataloaders, audio_net, evaluator, base_logger, writer, config, args, model_type, ckpt_path):
    ...
    # tensorboard
    # writer.add_graph(audio_net, next(iter(dataloaders['train']))['sentence_embeddings'].cuda())
    ...
    for epoch in range(config['EPOCHS']):
        ...
        for mode in ['train', 'validation', 'test']:
            ...
            # if config['CRITERION']['USE_SOFT_LABEL']:
            #     if mode == 'train':
            #         train_model_score = rho
            #     elif mode == 'test':
            #         test_model_score = rho
            # else:
            #     if mode == 'train':
            #         train_model_score = f1_score
            #     elif mode == 'test':
        ...
        if test_model_score >= test_best_score:
            ...
            if args.save:
                ...
                file_path = os.path.join(ckpt_path,'{}_{}_score-{:6.4f}.pt'.format(model_type, timestamp, test_model_score))
