...
def main(dataloaders, visual_net, audio_net, text_net, fusion_net, evaluator, base_logger, writer, config, args, model_type, ckpt_path):
    ...
    model_parameters = [*visual_net.parameters()] + [*audio_net.parameters()] + [*text_net.parameters()]  # + [*fusion_net.parameters()] + [*evaluator.parameters()]
    ...
    for epoch in range(config['EPOCHS']):
        ...
        # if epoch == 50:
        #     model_parameters = [*visual_net.parameters()] + [*audio_net.parameters()] + [*text_net.parameters()]
        #     optimizer, scheduler = get_optimizer_scheduler(model_parameters, config['OPTIMIZER'], config['SCHEDULER'])
            ...
            for data in tqdm(dataloaders[mode]):
                ...
                if mode == 'train':
                    ...
                    if config['OPTIMIZER']['USE_SAM']:
                        models = [visual_net, audio_net, text_net, fusion_net, evaluator]
...
if __name__ == '__main__':
    ...
    visual_net, audio_net, text_net, fusion_net, evaluator = get_models(config['MODEL'], args, model_type, ckpt_path)

    main(dataloaders, visual_net, audio_net, text_net, fusion_net, evaluator, base_logger, writer, config['MODEL'], args, model_type, ckpt_path)
