...
def main(dataloaders, visual_net, audio_net, evaluator, base_logger, writer, config, args, model_type, ckpt_path):
    ...
    # tensorboard
    # writer.add_graph(text_net, next(iter(dataloaders['train']))['sentence_embeddings'].cuda())
        ...
        for mode in ['train', 'test']:
            ...
            for data in tqdm(dataloaders[mode]):
                ...
                def model_processing(input):
                    ...
                    # features = (visual_features + audio_features) / 2
                    # median, output dim: (B, audio_features)
                    features = torch.median(torch.stack([visual_features, audio_features], axis=-1), dim=-1).values
                    # # maximum, output dim: (B, audio_features)
                    # features = torch.stack([visual_features, audio_features], axis=-1).max(dim=-1).values
